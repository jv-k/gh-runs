...
**The default costs no request, for any repository discovery reported it for.** `default_branch` rides the `/user/repos` payload R14's `permissions` and `archived` already ride ([repo-discovery](../repo-discovery/requirements.md) R7a), so the opener hands the branch to the form the same way it hands it the gate.

The form keeps its own resolver for the repository discovery has no branch for, and spends one `GET /repos/{owner}/{repo}` per open there, because R4 forbids answering with a guess about which ref will run. That is not only a transitional case: a local-store written before the field existed reloads without it and nothing refreshes it in place ([repo-discovery](../repo-discovery/requirements.md) R7a), so a session on such a store pays the request on every open until the store is rebuilt. The ref it lands on is correct either way.
