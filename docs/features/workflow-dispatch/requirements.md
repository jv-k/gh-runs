...
**The default costs no request.** `default_branch` rides the `/user/repos` payload R14's `permissions` and `archived` already ride ([repo-discovery](../repo-discovery/requirements.md) R7a), so the opener hands the branch to the form the same way it hands it the gate. A `GET /repos/{owner}/{repo}` per form open survives only as the fallback for a persisted record that predates the field, because R4 forbids answering with a guess about which ref will run.
