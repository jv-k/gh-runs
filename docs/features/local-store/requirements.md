...
**8 MiB, and why that number.** It is roughly 3.6x the largest legitimate response measured (a Runs listing at `per_page=100`, ~2.2 MB against `cli/cli`, one Run object being ~24 KB). The headroom is so that a repository busier than the reference one, or a Run object that grows a field, does not silently stop caching the Feed's own poll, which is a failure that costs a full 200 every cycle and is invisible. Base64-in-JSON makes the admitted entry about 10.7 MB, roughly a fifth of R15's total. It is independent of R15's bound because the two answer different questions: this one asks whether a response is a resource or a payload, R15's asks how much is kept. It is not a knob, by R9's argument transferred exactly as R15's bound inherits it.
