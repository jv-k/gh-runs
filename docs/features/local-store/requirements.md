...
**R16 is not enforced inside the store, and no requirement should imply that it is.** It is a property of what the tool requests: every listing asks for one page, and nothing crawls a repository's history into the store. There is no per-repository accounting at write time and there should not be, because by the time a response exists the window has already been chosen. An earlier wording of R15, and [ADR-0017](../../adr/0017-the-local-store-on-disk-contract.md) with it, described R16 as applying "at write time, before eviction is involved", which is true of no code. R25 is the store's own per-response bound and is the one that applies at write time, before this one.
...
**R25.** The local store must decline to hold a response body above a per-entry ceiling, and must decline to **buffer** it and not merely to save it. The ceiling is **8 MiB**, measured over the response body, a compiled constant and not a knob. Declining must never cost the caller its response: the bytes already read must be handed back in front of the untouched remainder, so the caller reads a complete body from a stream. A pre-existing entry for the same key must be left alone, to age out under R15's LRU rather than be removed.

**Buffering and saving are two costs, and declining to save reclaims only one.** The read that precedes the decision runs before the ETag is looked at, so a response the store then declines has already been held in memory in full. That is the cost with no other defence, and it is the reason the ceiling is expressed over the read rather than over the write.

**The rule keys on size and on nothing else.** A path rule was proposed and is not safe as stated: R14's `repoOf` yields `""` for any path whose first segment is not `repos`, and [repo-discovery](../repo-discovery/requirements.md)'s account enumeration is `/user/repos`, so a rule declining non-`/repos/` responses would decline the one response this store most wants to keep. A host rule separates the two correctly and costs the store a notion of "the API host" it does not have and would have to be given. Size needs nothing the store is not already holding, and it covers a response shape nobody has thought of yet.

**The size must be measured and not read from `Content-Length`.** go-gh sets no `Accept-Encoding`, so `net/http` adds `gzip` itself, decodes transparently, and deletes `Content-Length`. The header is therefore absent for gzipped JSON, which is every response worth keeping, and present for binary downloads, which are the responses this ceiling exists to refuse. A header rule would be reliable in exactly the wrong direction.

**8 MiB, and why that number.** It is roughly 3.6x the largest legitimate response measured (a Runs listing at `per_page=100`, 2,311,840 bytes or ~2.20 MiB against `cli/cli`, one Run object being ~23 KiB). The [PRD](../../PRD.md)'s constraints table is where those measurements live, and this figure is priced off them rather than restating them. The headroom is so that a repository busier than the reference one, or a Run object that grows a field, does not silently stop caching the Feed's own poll, which is a failure that costs a full 200 every cycle and is invisible. Base64-in-JSON makes the admitted entry about 10.7 MiB, ~21% of R15's total. It is independent of R15's bound because the two answer different questions: this one asks whether a response is a resource or a payload, R15's asks how much is kept. It is not a knob, by R9's argument transferred exactly as R15's bound inherits it.

**What the ceiling leaves open, deliberately.** It bounds the entry cost rather than eliminating it. A blob response *under* 8 MiB is still saved, and where its URL addresses no repository it is still written with `Repo: ""`, which R14's invalidation can never reach, so it survives until LRU evicts it. That residue is a few kilobytes against R15's 50 MB, which is why size alone is enough here and the host rule was not worth what it costs. Closing it entirely is the client-side choice's job, and the two together are the defence in depth the paragraph below describes.

**This is a store-side rule and it does not replace the client-side choice.** [ADR-0012](../../adr/0012-transport-chain-and-the-client-surface.md)'s two entry points stand: a request whose response is a binary download still enters the chain below the store. The two are defence in depth. The convention alone was invisible from inside the store, enforced only by review, and has been got wrong three times.
...
**AC21: The ceiling bounds the read, not only the write.** Given a 200 carrying an ETag whose body is many times the ceiling, at the instant the transport returns and before the caller has read anything, exactly the ceiling plus one byte have been read from the response, the store holds no entry for it, and any entry it already held for that key is byte-identical to what it was. The caller then reads the body in full, byte for byte, and closing it closes the underlying stream. A body exactly at the ceiling is saved and a body one byte past it is not. A body under the ceiling is saved, carries its repository tag, and is served back re-readable, as it was before this criterion existed (R25).

The read count is what makes this criterion mean anything, and it cannot be observed on a cassette, because go-vcr replays from a buffer and every byte is already in memory before the store runs. It is observed against a fake base whose body counts what it hands out, on R18's own terms: a cassette proves what the API said, and this proves what the store did with it.
