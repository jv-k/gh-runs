...
**R7.** `--json` MUST accept at least gh's field set: `attempt`, `conclusion`, `createdAt`, `databaseId`, `displayTitle`, `event`, `headBranch`, `headSha`, `name`, `number`, `startedAt`, `status`, `updatedAt`, `url`, `workflowDatabaseId`, `workflowName`. `-q/--jq` MUST operate over that output exactly as gh does, byte-identical through go-gh's own jq. `-t/--template` MUST operate over that output with a documented subset of gh's template functions ([ADR-0023](../../adr/0023-the-template-function-subset.md)): gh's pure, deterministic funcs (`join`, `pluck`, `timefmt`, `truncate`, `hyperlink`, `timeago`, and the sprig string funcs `contains`, `hasPrefix`, `hasSuffix`, `regexMatch`, `replace`) MUST match gh, and `color`, `autocolor`, `tablerow` and `tablerender` are unsupported and MUST error by name. The four are dropped because go-gh's template package cannot be imported against this module's v2 pin ([ADR-0013](../../adr/0013-dependency-pins.md)), and reproducing them needs a color library or go-gh's cellbuf-backed `tableprinter`. Additional fields MAY be offered. No listed field may be omitted or renamed.
...
**R28.** The surface MUST offer `gh runs cancel` and `gh runs rerun`, the non-interactive form of [run-lifecycle](../run-lifecycle/requirements.md)'s five operations. `--force` selects force-cancel and `--failed` selects re-run failed Jobs, each a distinct operation against a distinct endpoint (that document's R6 and R13), `-j/--job` selects re-run one Job (that document's R14a), and `-d/--debug` is R14's opt-in on all three re-runs. Every spelling is gh's own, taken one form at a time: `gh run cancel --force`, `gh run rerun --failed`, `gh run rerun -j <id>`, and `-d/--debug` on either re-run form. [ADR-0008](../../adr/0008-full-cli-surface-despite-gh-overlap.md) commits to mirroring each of them, and `--failed` and `-j` are alternatives rather than a pair.

**R28a.** `-j/--job` MUST take a Job id and re-run the Job that id names, which is gh's argument and gh's meaning exactly, dependent Jobs included ([run-lifecycle](../run-lifecycle/requirements.md) R14a). With `-j` carried, no `gh run rerun` flag is left unmirrored and [ADR-0008](../../adr/0008-full-cli-surface-despite-gh-overlap.md)'s drop-in-superset claim holds without exception. A Job id names one Job of one Run and so names no repository, which puts `-j` under R29's rule for a bare Run ID: it MUST be refused under fan-out, where there is no repository to address the Job endpoint against.

**R28b.** `--job-name` MUST take a Job name and re-run the Job of that name in each Run of the resolved set, at most one per Run ([run-lifecycle](../run-lifecycle/requirements.md) R14a). A name matching no Job in a given Run MUST be a skip with a stated reason, and the invocation MUST still exit 0 on that ground alone. gh addresses one Run at a time and so has no by-name selector to mirror, which puts this flag where [ADR-0022](../../adr/0022-the-cli-beyond-gh-precedent.md) places every extension: at a point where parity is silent.

A Run that matched no Job has no Item to carry that skip, and which layer owns it is settled in [ADR-0019](../../adr/0019-ops-plan-and-confirmed.md), amended: the frozen set gains an Item-less member carrying the tuple and the reason, so the report stays in the one place this requirement puts it. The mechanism is that ADR's and this requirement stays about the observable behaviour. A resolution cut short before it reached every Run is a different outcome from a name that matched nothing, and it MUST exit 1 under R17 rather than 0 under this requirement.

**R28c.** `-j/--job` and `--job-name` MUST be separate flags and MUST NOT be merged into one flag whose argument is read as an id or a name by context. An operator pasting a working gh invocation would have its Job id read as a name, match nothing, skip, and exit 0 having issued no write, which is R16's dishonesty about counts applied to a write. Being separate, the two MUST also be refused together, on R29's grounds for a positional Run ID beside a filter: they select one operation's target two ways, and no reading of the pair is the operator's stated intent.
...
- **Resolved: `-t/--template` ships a documented subset of gh's template funcs, not full parity.** go-gh's `pkg/template`, where gh's funcs live, cannot be imported against this module's `charm.land` v2 pin ([ADR-0013](../../adr/0013-dependency-pins.md)). gh's pure funcs are reimplemented in-repo and match gh. `color`, `autocolor`, `tablerow` and `tablerender`, which need a color library or go-gh's cellbuf-backed `tableprinter`, are dropped and error by name. R7 and AC21 carry it, [ADR-0023](../../adr/0023-the-template-function-subset.md) records the options it beat (accept the gap, dependency surgery, a library-free `color` behind a TTY check).
...
- [ADR-0023: The -t template function subset](../../adr/0023-the-template-function-subset.md)
