...
**R28.** The surface MUST offer `gh runs cancel` and `gh runs rerun`, the non-interactive form of [run-lifecycle](../run-lifecycle/requirements.md)'s five operations. `--force` selects force-cancel and `--failed` selects re-run failed Jobs, each a distinct operation against a distinct endpoint (that document's R6 and R13), `-j/--job` selects re-run one Job (that document's R14a), and `--debug` is R14's opt-in on all three re-runs. The spellings are gh's own on `gh run cancel --force` and `gh run rerun --failed --debug -j`, which [ADR-0008](../../adr/0008-full-cli-surface-despite-gh-overlap.md) commits to mirroring.

**R28a.** `-j/--job` MUST take a Job id and re-run that one Job, which is gh's argument and gh's meaning exactly. With `-j` carried, no `gh run rerun` flag is left unmirrored and [ADR-0008](../../adr/0008-full-cli-surface-despite-gh-overlap.md)'s drop-in-superset claim holds without exception.
...
`-j/--job` and `--job-name` MUST be separate flags and MUST NOT be merged into one flag whose argument is read as an id or a name by context. An operator pasting a working gh invocation would have its Job id read as a name, match nothing, skip, and exit 0 having issued no write, which is R16's dishonesty about counts applied to a write.
