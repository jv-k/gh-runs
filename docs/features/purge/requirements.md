...
**R12.** A Purge MUST exclude Runs whose Status is not `completed`, record them as skipped, and MUST NOT cancel them to make them deletable. DELETE rejects a Run that is still in progress (**inferred, never documented**: see open question 4), and cancelling is a Run lifecycle operation the user invokes deliberately. **R12 decides on crawl-time Status, and nothing revalidates it before the DELETE.** That is deliberate, and the reasoning is below.
...
**R15.** A Purge MUST show continuous progress: Runs deleted against the frozen total, skips and failures so far, the current delete rate, elapsed time, and remaining time. While the throttle's observed rate and its ceiling disagree, remaining time MUST be presented as a **range**, its optimistic end computed from the governor's current dynamic ceiling and its pessimistic end from the observed trailing rate, clamped never worse than the governor's floor. When both ends round to the same displayed figure, the display MUST collapse to that single figure, explicitly presented as an estimate. Display granularity is the collapse rule, and a timer MUST NOT be.

**A point figure during the ramp is the shape success criterion 6 forbids.** The throttle starts at 1/sec and ramps across a 3× band, so a single figure computed in the first minute can be hours wrong, and an "estimate" label does not make it honest, any more than a caption under `total_count` makes 18,258 the number of Runs a filtered listing can reach. The count side of this line is exact (R3's crawl), and the time side is the one guess on screen, so the guess says its spread: a range from the governor's own bounds always contains the truth, which is the [PRD](../../PRD.md)'s actual demand. Every input already exists (the frozen count, the ceiling, the observed rate, the floor), so the range is arithmetic rather than mechanism, and the collapse rule adds no constant: the range folds exactly when its ends stop differing at the precision the person can see. The floor clamp keeps the pessimistic end honest in the other direction, because a trailing rate observed over the first handful of deletes can dip below any rate the governor can actually sustain.
...
**R21.** A Purge MUST stop itself when consecutive failures reach the breaker threshold. A single success MUST reset the counter, and a backoff under R19 MUST NOT advance it. Sustained consecutive failure means something systemic (a revoked token, an archived repository), and grinding on for another 90 minutes helps nobody. The summary MUST say the Purge circuit-broke, and why. The threshold is settable: [settings](../settings/requirements.md) R21 owns the key, the default and the clamp at both ends, and there is no value at which the breaker never fires.
...
**AC13: The circuit breaker counts consecutively.** Against the default breaker threshold, given 49 consecutive failures followed by one success followed by 49 more failures, the Purge does not stop. Given 50 consecutive failures, it stops, issues no further DELETE, and the summary names the circuit-break. Given a config setting the threshold to 1, the first failure stops it, and given one setting it to 5,000, the clamp holds it at 500 ([settings](../settings/requirements.md) R21, AC13).
...
**AC22: The frozen set is inspectable, and inspecting it costs nothing.** Given a frozen set of 18,258 Runs drawn from several repositories, the confirm modal names the keystroke that opens the inspect view. Opening it, paging to the last row and closing it issues zero HTTP requests and zero DELETEs. Every row that names a write carries its owning repository, its Run ID, and Status and Conclusion in separate cells, with Conclusion empty on any row whose Status is not `completed`. Those rows are the same tuples `Execute` is handed, the last of them is the oldest Run in the set by `run_started_at`, and no key in the view selects, deletes or confirms.

The qualifier is [ADR-0019](../../adr/0019-ops-plan-and-confirmed.md)'s amended second kind of member, and it narrows this criterion rather than weakening it. A frozen set for a by-name per-Job re-run may hold a member that resolved to no object ([cli-surface](../cli-surface/requirements.md) R28b). Such a row names the absence of a write: it is handed to nothing, it holds no Run and so has neither a Status nor a Conclusion to put in a cell, and it carries no `run_started_at` to sort on. It renders its repository and its Run ID, leaves the two enum cells empty on the same reading that already empties Conclusion for a Run that is not `completed`, and takes no part in the ordering. This is the same narrowing `KindWorkflow` and `KindJob` arrived inside for the deletion log's kind column, and not a fresh exception to it. Closing it leaves the frozen set and the confirmation state untouched, and the Purge still requires R7's typed count. The fixture's own totals are what the assertion reads, never a constant compiled into the test (R30).

**AC23: Remaining time is a range until the ends agree.** Given a Purge over R26's fixtures with R27's injected clock, while the observed rate and the governor's ceiling disagree beyond display granularity, the progress line shows a remaining-time range whose optimistic end derives from the current dynamic ceiling and whose pessimistic end never exceeds the remaining count divided by the governor's floor. Advancing virtual time until both ends round to the same displayed figure collapses the display to that single figure, labelled an estimate, and no timer and no configuration participates in the switch (R15).
...
4. **Resolved: nothing is documented, so R12 stays conservative on the record rather than by omission.** GitHub's OpenAPI description for `DELETE /repos/{owner}/{repo}/actions/runs/{run_id}` declares exactly one response, `204`, and neither it nor the rendered reference page mentions Status, run state, or a `409`. The only public evidence that a restriction exists is gh's own client, which handles a `409 Conflict` and labels it "cannot delete a workflow run that is completed". **That label cannot be right**, because deleting completed Runs is precisely what `gh run delete` exists to do, and the likely intent is "not completed", which agrees with R12. The trigger set stays unknown: `queued`, `waiting`, `requested` and `pending` are each not-completed and would all be rejected under the inferred rule, but inference is not measurement, and measuring means issuing a live DELETE against a real Run, which this project forbids. **R12 does not narrow.** Full citations in [delete-run-status-restriction.md](../../research/delete-run-status-restriction.md).
...
6. **Resolved: the breaker threshold is settable, defaulting to 50 and clamped to 1 to 500.** [settings](../settings/requirements.md) R21 owns the key (`purge_breaker_failures`), the default and both bounds. R21 here keeps the behaviour, and the two AC13s pin the clamp from each side.

    **The key passes settings R13's intent test where the poll interval fails it.** "How much sustained failure before my Purge gives up" is a trade between an early stop and a grinding one, answerable from the person's own context. And the flaky-network worry is smaller than it reads, because of what advances the counter: rate limits never do (R19), a 404 is a success (R18), and one success resets it. Reaching even the default takes 50 R20 outcomes in an unbroken row, under a minute of total failure at the throttle's reachable rates, which is an outage or a revoked token rather than flakiness.

    **The bounds follow R8's inversion.** Raising the threshold lowers protection, so the floor under protection is a hard upper bound of 500, clamped with a diagnostic. The floor is 1, and there is no off value: a breaker that never fires plus a revoked token is a Purge issuing 403s for 10 hours, which is the harm the breaker exists to stop. 50 and 500 are chosen rather than measured, on the same footing as question 1's numbers.
...
8. **Resolved: neither yields ([ADR-0021](../../adr/0021-the-scheduler-cadence-policy.md), owned by [polling-scheduler](../polling-scheduler/requirements.md) open question 5).** The rate-governor's five-minute burn window amortises the crawl's one-off burst below the pressure predicate, staged demotion answers genuine pressure whoever caused it, and the governor's dynamic write ceiling subtracts crawl reads automatically. One residual is this document's to raise if its numbers ever demand it: an account far larger than the reference could carry a crawl that exceeds ~900 points/min on reads alone at full tilt, which today rides [ADR-0018](../../adr/0018-the-fanout-concurrency-shape.md)'s exhaustion path rather than being paced in advance.
9. **Resolved: a range while the throttle ramps, a point figure once display granularity says the range is one.** R15 carries the amended wording and AC23 pins it. The optimistic end is the governor's current dynamic ceiling, the pessimistic end is the observed trailing rate clamped at the governor's floor, and the range collapses to a single labelled estimate when both ends round to the same displayed figure.

    **The collapse rule is display granularity deliberately.** A fixed ratio would be one more asserted constant needing a home, and a timer would encode the ramp's shape somewhere the governor cannot update. There is no "estimating" placeholder either: the ceiling end is computable from the first classified response, and an early range is honestly wide rather than absent.
