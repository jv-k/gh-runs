...
**Undecided: is Reclamation per repository, or a cross-repo rollup?** Written here for one repository at a time, matching the endpoint's shape. A rollup is affordable (one request per repository, over a fan-out [ADR-0003](../../adr/0003-multi-repo-via-client-side-fanout.md) already builds), and "which of my 163 repositories is hoarding Caches?" is a question the per-repo view cannot answer. It was not decided.
