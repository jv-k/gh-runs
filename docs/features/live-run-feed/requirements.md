...
**The axis also takes `repo:this-repo`, which is the Feed's answer to a scope key** ([settings](../settings/requirements.md) R19's note, issue [#117](https://github.com/jv-k/gh-runs/issues/117)). It states the repository of the working directory rather than a name, and the Feed resolves it where it matches, against the same `CurrentRepo` resolver the Workflows and Storage tabs take for their own this-repo scope. The Feed holds the token as stated, not as resolved, so the filter input and the filter label keep reading `repo:this-repo`: a pin the operator typed must not decay into a name under them, and moving directory must change what matches without changing a character on screen.

**Where the working directory has no repository, the Feed MUST say so.** The token then contributes nothing and the view widens to every repository, which is [settings](../settings/requirements.md) R19's stated fallback rather than an empty view. It is said in the filter chrome, immediately beneath the line that states the active filter, because that line exists to make a narrowing the operator did not type visible, and a pin that is not in force must not read the same as one that is. It is a line of its own rather than a clause on that line: the two together exceed R4a's 100 columns and the fallback was truncated away, which [ADR-0016](../../adr/0016-the-filter-representation.md)'s amendment records. The Workflows tab's scope label states the same fallback in the same terms.
