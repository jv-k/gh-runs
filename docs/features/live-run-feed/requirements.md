...
**Where the working directory has no repository, the Feed MUST say so.** The token then contributes nothing and the view widens to every repository, which is [settings](../settings/requirements.md) R19's stated fallback rather than an empty view. The line that states the active filter is where it is said, because that line exists to make a narrowing the operator did not type visible, and a pin that is not in force must not read the same as one that is. The Workflows tab's scope label states the same fallback in the same terms.
