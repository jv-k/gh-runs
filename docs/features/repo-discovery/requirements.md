...
2. **Telling a rate-limit 403 from an authorization 403 is UNKNOWN.** R10 requires a 403 to be surfaced as an authorization outcome. [ADR-0007](../../adr/0007-adaptive-delete-throttle.md) requires a 403 to trigger hard backoff. Both are canon and they send the same status code to opposite handlers. Misreading it either way is costly: an authorization 403 treated as a rate limit stalls a Purge behind a backoff that will never clear, and a rate-limit 403 treated as authorization keeps issuing requests and risks a block. Shared with [rate-governor](../rate-governor/requirements.md) and [purge](../purge/requirements.md), and it must be resolved by measurement before either ships.
