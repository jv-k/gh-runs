...
4. **Half resolved: pressure now has an onset, and the staging between tiers does not.** [rate-governor](../rate-governor/requirements.md) R8a defines pressure as projected exhaustion (`remaining / burn < time_to_reset`) rather than as a percentage, so R15's *first* demotion has a moment and this scheduler reads it off the Budget Readout rather than deriving it (R14). What stays open is whether all three tiers demote at that moment or in stages, and what separates the stages if they do.

    **The projection also introduces a loop this question now owns.** Demoting cuts the read rate, which cuts burn, which clears the predicate, which would promote the tiers back into the same pressure. R8a's five-minute window damps it and does not answer it. Whether demotion needs hysteresis, a dwell, or a rule that a demotion holds until the reset, is undecided.
