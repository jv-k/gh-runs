...
7. **Resolved: the scheduler does not own it ([ADR-0021](../../adr/0021-the-scheduler-cadence-policy.md)).** "Fits no tier in R5" was the answer rather than the problem: tiers are steady-state, repository-level policy computed from Status and visibility (R4, R6), and an expectant poll is event-driven, short-lived and bound to one user action. If [workflow-dispatch](../workflow-dispatch/requirements.md)'s `return_run_details` verification leaves the race standing, the dispatch path owns a bounded, pane-owned fetch loop per [ADR-0015](../../adr/0015-the-async-model.md)'s shape, riding the same transport chain (limited by [ADR-0018](../../adr/0018-the-fanout-concurrency-shape.md), accounted by the governor, and discardable under R10 when its dispatch context has passed). Whether the loop is needed at all, and its bounds, stay with workflow-dispatch.
