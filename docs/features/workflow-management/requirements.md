...
**Resolved: no special handling, and it likely never paints (R3).** `disabled_fork` is in the official enum and was never observed across ~80 forks, and a fork with Actions un-enabled serves `{"total_count": 0, "workflows": []}` rather than a `disabled_fork` row (despite carrying 8 to 24 workflow YAML files on disk, `lee-dohm/cli` and `openCEC/CEC-IDE` among them), so the state may be unreachable through this endpoint entirely. R3 carries it either way: an unrecognised state renders verbatim, a state that never arrives needs no handling, and if one ever appears the enable operation is offered best-effort. This is not answered by disabling Actions on a fork to find out.

**Resolved: treat it as non-exhaustive (R3).** The five values are the ones observed, and R3 renders any unrecognised state verbatim, so exhaustiveness is not load-bearing and needs no measurement.

**Resolved: no, disabling does not cancel a running Run (R5).** GitHub documents disabling as stopping a Workflow from being triggered, not as cancelling Runs already in flight, so the disable confirmation need say nothing about them, and R5 now states it. R8 reflects the API's reported state regardless. This is documented behaviour rather than a measurement here, since confirming it means a live disable against a Workflow with a Run going.

**Resolved: not load-bearing.** The Workflow list is refreshed on demand rather than continuously polled, and R8 re-reads the state after a toggle, so conditional revalidation ([ADR-0004](../../adr/0004-conditional-polling-for-liveness.md)) is an optimisation this surface does not require. Left unmeasured, the same call [storage-reclamation](../storage-reclamation/requirements.md) makes for its own list.

**Resolved: prevalence does not gate the design.** R11 to R13 stand whether Orphaned Runs are a headline cleanup win or a rare curiosity, because the capability exists either way and identifies them from Workflow state alone (R12). How many the reference account's 163 repositories carry is a product-value question, not a build-gating one, and it changes no requirement.
...
**Resolved: yes, and cli-surface owns the spelling.** The non-interactive surface covers enable and disable and the Workflow list, on ADR-0008's standalone-coherence argument: a user without `gh` installed has no `gh workflow enable` to fall back on. The exact command and flag spelling belong to [cli-surface](../cli-surface/requirements.md), consistent with its superset commitment.
