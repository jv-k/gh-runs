...
1. **Resolved by body, not by header, and it fails safe toward backoff.** R12 sends a 403 to hard backoff. [repo-discovery](../repo-discovery/requirements.md) R10 and [purge](../purge/requirements.md) R20 send a 403 to an authorization outcome. Both are canon, and they demand opposite handling of the same status code.
    ...
    **"The endpoint's own reference page" is the parent resource's page on the Actions sub-action endpoints.** `POST .../runs/{id}/cancel`, `POST .../runs/{id}/rerun`, `PUT .../workflows/{id}/enable` and `PUT .../workflows/{id}/disable` end in the action rather than in the resource, and GitHub documents each of them on the workflow-runs or workflows page. Their authorization `documentation_url` therefore carries the parent's noun and never the segment the request path ends in, so the correspondence has to be tested against the noun behind the action. Matching only the path's last segment sends every one of these to the rate-limit direction: safe, and bounded by [purge](../purge/requirements.md) R19a, but it spends three backoffs and their waits on the case [purge](../purge/requirements.md) R13 calls expected. Closed in the classifier (issue #68), where the list of sub-actions lives.
