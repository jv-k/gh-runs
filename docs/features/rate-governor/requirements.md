...
**R2.** The governor must be the single authority for write throughput. Every write must be paced by it: Run deletion, log deletion ([log-viewer](../log-viewer/requirements.md) R17), cancel, force-cancel, re-run, re-run-failed-jobs, Dispatch, Workflow enable and disable ([workflow-management](../workflow-management/requirements.md) R5), Cache and Artifact deletion, and the two approval writes, the fork-PR approve ([approvals](../approvals/requirements.md) R11) and the pending-deployment review ([approvals](../approvals/requirements.md) R12). That is thirteen writes, and the list is exhaustive rather than illustrative. A write this requirement does not name is a gap in the list, never a write outside it.

**The approval writes are writes, and the list did not have them either.** `POST /actions/runs/{id}/approve` is [approvals](../approvals/requirements.md) R11's and `POST /actions/runs/{id}/pending_deployments` is R12's, `ops` owns both, and both spend the same 5 points R16 prices every other write at. Re-run-failed-jobs is named here for the same reason: [run-lifecycle](../run-lifecycle/requirements.md) R13 holds it apart from re-run as its own endpoint, so a list that says "re-run" and means two endpoints is short by one. As with enable and disable, the omission never cost a request, because [ADR-0012](../../adr/0012-transport-chain-and-the-client-surface.md) puts the governor under every request as a RoundTripper that discriminates on method. What it cost was the list's claim to be exhaustive, and a classifier reading this list as complete would have been wrong three times. Found while closing issue #68.
...
1. **Resolved by body, not by header, and it fails safe toward backoff.** R12 sends a 403 to hard backoff. [repo-discovery](../repo-discovery/requirements.md) R10 and [purge](../purge/requirements.md) R20 send a 403 to an authorization outcome. Both are canon, and they demand opposite handling of the same status code.
    ...
    **"The endpoint's own reference page" often does not carry the endpoint's last path segment, and the correspondence must be tested against what the page's path does carry.** Two shapes break a naive last-segment match, and between them they cover most of R2's writes. A segment naming an action (`POST .../runs/{id}/cancel`, `PUT .../workflows/{id}/enable`) or a sub-resource (`DELETE .../runs/{id}/logs`, `GET .../cache/usage`, `POST .../runs/{id}/pending_deployments`) is documented on the PARENT resource's page, so the `documentation_url` carries workflow-runs, workflows or cache instead. And a Cache is documented at `/rest/actions/cache` while its endpoint says `caches`, a plural the page's path does not carry. Matching only the last segment sends every one of these to the rate-limit direction: safe, and bounded by [purge](../purge/requirements.md) R19a, but it spends three backoffs and their waits on the case [purge](../purge/requirements.md) R13 calls expected, halving the write ramp toward its floor on each one. Closed in the classifier (issue #68), which steps over the parent-documented segments and tries the other number, and which keeps the safe direction for any segment it does not know.

    **How much that buys depends on which 403 GitHub sends, and only one of the two is helped.** The correspondence can only be tested when the `documentation_url` is the endpoint's own reference page, which is the shape open question 1 measured. GitHub also answers a fine-grained PAT with the general `/rest/overview/permissions-required-for-fine-grained-personal-access-tokens` page, which names no resource in any number, and the cassettes in `internal/ops/testdata` and `internal/cli/testdata` record exactly that. Those 403s still classify as rate limiting and still cost R19a's three attempts. That is the bound doing its job rather than a gap to close: matching a page that names no endpoint would mean guessing, and open question 1 refuses to guess in the direction that keeps issuing. What the fix removes is the wasted backoff on the reference-page variant, and R19a remains the only thing that ends the other.

    **A repository name is not a resource noun, and the classifier must not treat it as one.** `GET /repos/{owner}/{repo}` has no sub-path, so its last segment is a name the user chose, and `GET /repos/{owner}/{repo}/contents/{path}` puts a user-chosen file path in the same position. A correspondence test that compared those against the `documentation_url` would let a repository named `rest`, `overview` or `resources` turn a genuine secondary limit into an authorization reading, because every REST doc URL carries `rest` and the legacy rate-limit pages carry the rest. That error runs in the one direction this component exists to prevent: no backoff, no hold, and [repo-discovery](../repo-discovery/requirements.md) R22's adoption issuing straight into the limit. Only a known resource noun may correspond, and it is compared against whole path segments rather than as a substring, so the safety is a property of the classifier rather than of how GitHub happens to word a URL.
