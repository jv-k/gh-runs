...
| **Filtered listing silently caps at 1,000.** `total_count` still reports 18,260 on the reachable pages, and page 11 returns `[]` with no error. Unfiltered listing reaches ~27k+. | Never trust `total_count` in a filtered view. Live Feed filters server-side and labels honestly, and a Purge crawls unfiltered. → [ADR-0005](./adr/0005-hybrid-filtered-live-unfiltered-purge.md) |
| **`Link`'s `rel="last"` lies in a filtered view exactly as `total_count` does, and past the cap `total_count` collapses to 0.** Measured on `cli/cli`, `status=success`: page 1 claims `total_count` 18,256 and `rel="last"` **page 183**, while only 10 pages are ever served. Page 11 returns HTTP 200, `[]`, and **`total_count: 0`**, not 18,256. | `rel="last"` is derived from `total_count` and inherits its dishonesty, so it cannot detect the cap either. Neither number may seed a crawl. The 0 is not exhaustion, it is the cap. |
