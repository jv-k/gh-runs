...
| **Response bodies span three orders of magnitude, and the largest legitimate one is the Feed's own poll.** Measured on `cli/cli`, GETs only: a Runs listing at `per_page=100`, the scheduler's own poll shape, is **2,311,840 bytes (2.20 MiB)**; one Run object is 23,882; `user/repos?per_page=100` is 518,289; a Workflows listing at `per_page=100` is 8,994. | The local-store's most valuable entry, the one the free cold start rests on, is also its biggest, so a per-entry ceiling has to be priced above it rather than at a round number. [local-store](./features/local-store/requirements.md) R25 sets 8 MiB, ~3.6x the listing. Written base64-in-JSON at ~1.33x, a poll set of ~26 repositories that busy passes R15's 50 MB total on Run listings alone, so LRU eviction is on the ordinary path rather than in a corner of it. → [ADR-0017](./adr/0017-the-local-store-on-disk-contract.md) |
...
| **Dispatch with `return_run_details: true` returns 200 with the Run ID. Measured, and the 204-no-Run-ID canon is superseded.** GitHub's OpenAPI spec (v1.1.4, committed 2026-07-14) added a `return_run_details: boolean` body parameter to "Create a workflow dispatch event". A 200 returns `{workflow_run_id, run_url, html_url}`, all three required, when it is `true`, and the 204 is the `false` path. A live dispatch confirmed the 200 path (#27). | The Run ID comes from the Dispatch response, so [workflow-dispatch](./features/workflow-dispatch/requirements.md) R16 sends the parameter and R17 to R19's correlation poll is retired rather than hedged. Correlation survives only as deferred notifications' 2.1 starting point. Verbatim in [docs/research/workflow-dispatch-measurements.md](./research/workflow-dispatch-measurements.md). |
