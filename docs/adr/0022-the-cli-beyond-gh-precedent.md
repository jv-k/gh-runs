...
**`-j/--job <id>` is parity, and it closes the last gap.** It re-runs the single Job that id names, which is exactly what `gh run rerun -j` does with exactly the argument gh takes. [ADR-0008](./0008-full-cli-surface-despite-gh-overlap.md) committed the CLI to being a drop-in superset of `gh run`, and this was the only flag standing between it and that claim. Nothing about it needs this ADR: it is R2's parity working.
