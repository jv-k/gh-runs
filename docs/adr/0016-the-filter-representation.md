...
**The marker's word is `this-repo`,** the one `workflows_scope` and `storage_scope` already use for the same idea. It carries the axis name where the axis needs naming and not otherwise, exactly as a named repository does: `repo:this-repo` in the input's grammar, and a bare `this-repo` entry in `launch_filter.repos`, where the key already says which axis it is. `ParseQuery` sets the marker, `QueryString` renders it, so a filter carrying it round-trips exactly as a named entry does. The token cannot collide with a repository reference, because `domain.ParseRepoRef` requires two segments and this has one.
...
**The say-so obligation travels with the capability.** Where the marker resolves to nothing, the axis contributes nothing and the Feed states the fallback in its filter chrome, which is R19's rule for the other two tabs applied to the surface that now has their capability.

**Amended 2026-07-27 (#147): the fallback is its own line, beneath the filter line.** This section, [settings](../features/settings/requirements.md) R19's note and [live-run-feed](../features/live-run-feed/requirements.md) R3 all first said the fallback is stated *in* the line that states the active filter. Measured at implementation, it does not fit there. The filter line already carries the query and the two key hints, which for the marker alone is `filter repo:this-repo  (/ edit, then esc clears)`, and the fallback sentence is 79 columns on top of that against [live-run-feed](../features/live-run-feed/requirements.md) R4a's 100-column floor. The line is clamped, so the sentence was truncated off the end of it, and a say-so obligation discharged by a message the operator cannot read is not discharged. It is a second chrome line, counted by `chromeLineCount` like every other conditional statement the Feed makes, and it costs a row only while the pin is not in force.
...
**`Filter` now holds a value `Match` alone cannot evaluate.** `Workflow` is also an unresolved selector, but only its server half needs the outside world: client-side it matches the stamped name or ID and needs nothing. The marker is the first field whose **client** predicate is undetermined until something outside the struct supplies a value, and it is bounded on purpose: one field, one resolution method, `Match` widening where it is unresolved. A second such field would be a decision, not a diff, because the property that every `Match` arm is self-contained is what the first one spends.
