...
└── internal/
    ...
    ├── workflowlist/        One repository's Workflow listing. A tab, an engine and the CLI, see below.
...
| `workflowlist` | `domain`, `ghlink` | everything but `domain` and `ghlink` |
...
**`workflowlist` is a package for the reason `filter` is one, and it arrived the way `timefmt` did.** One repository's Workflow listing is the name-to-id map three surfaces need: the Workflows tab renders it ([workflow-management](../features/workflow-management/requirements.md) R1), the scheduler joins it against each Run's `workflow_id` to stamp `WorkflowState` ([run-detail](../features/run-detail/requirements.md) R8, [ADR-0014](./0014-domain-types-and-the-budget-readout.md)), and `cli` resolves `-w NAME` and the `workflowName` column through it ([cli-surface](../features/cli-surface/requirements.md)). It lived in `tui/workflows` while the tab was its only consumer, which put it where `cli` may never look and left `main.go` handing a tab's constructor to the engine. Nothing in it was tab-shaped: no model, no message, no key binding. So it is `internal/workflowlist`, over `domain` and `ghlink`, and the tab keeps the model, the `WorkflowsFetched` message and the rendering. It takes a narrowed `Requester` rather than importing `ghclient`, which is the same seam `store` and `ghclient` hold between them: `main.go` is the only place that knows both. Issue #95 made the move, and it amends this ADR in place exactly as issue #123 did when `timefmt` was promoted out of `cli`.

**`config` carries the fifth arrow onto `filter`, and it is the same cheap one.** [settings](../features/settings/requirements.md) R9 requires the launch filter to be stored as [ADR-0016](./0016-the-filter-representation.md)'s structured Filter, with Status and Conclusion as distinct typed fields, so the loaded `Config` holds a `filter.Filter` and `config` imports `filter`. The alternative is a config-private mirror of the type, which is two representations of one value and the drift [ADR-0014](./0014-domain-types-and-the-budget-readout.md) and ADR-0016 both exist to prevent: the mirror would need its own validation, its own permissive-pair rule, and a conversion nobody could test against the real one. The arrow points at a package that imports `domain` alone and holds no I/O, which is the shape this table already priced as cheap for the scheduler, and it creates no cycle: `filter` may never import `config`, and `palette`, the one package below `config`'s consumers that imports it, is unaffected. `config` still imports nothing else.
