...
**A deletion holds R29's log exclusively, whichever entry it came in through.** Two deletion walks at once open two append handles to one file, each tracking size from its own `Stat`, so one handle's rotation renames the file the other is appending to and a generation can vanish. That file is the one thing recoverable from nowhere else, so a second concurrent deletion refuses to start and names the log, exactly as an unwritable log makes it refuse. Non-deletion walks write no log and are not gated: a Workflow toggle beside a running Purge touches nothing shared. The live case is not hypothetical, because [log-viewer](../features/log-viewer/requirements.md) R17's log deletion reaches `Execute` while the TUI's Purge is streaming.
