...
**The two readers share the code and not the list.** `main.go` fills the engine's seam with the same reader the tab uses, so there is one place that knows the endpoint and its envelope, but the tab still runs its own fan-out and holds its own copy, and the engine's map is never handed to it. That is a duplicate request, not a duplicate decoder. Issue #95 moved that reader out of the tab into `internal/workflowlist`, for the reason [ADR-0011](./0011-package-layout-and-dependency-direction.md) gave `filter` its own package, so the CLI's blocked `-w NAME` and `workflowName` consumers can reach it without importing `tui`. The duplicate request is untouched by the move and the catalog member is what closes it: the day the tab consumes the engine's copy, the event is one `emit` away. The stamped join does not wait on either.
...
**The frame carries the governor's write bounds, and that is why it can.** [purge](../features/purge/requirements.md) R15's remaining-time range is computed between the current dynamic ceiling and the floor, so `ops` reads both through a `Pacer` seam `main.go` fills with the governor and stamps them on the frame. Doing it that way rather than adding them to the broadcast `Readout` keeps a float that moves with every read out of a message whose whole delivery rule is "broadcast on change", and it leaves the surface holding pure arithmetic over one value, which is what makes AC23 checkable from a fabricated frame. The frame also carries the frozen set's `Kind`, because the `Operation` alone does not name what is happening: [CONTEXT.md](../CONTEXT.md) makes a Purge a filtered bulk deletion of **Runs**, so the same `OpDelete` over Caches and Artifacts is a Reclamation and a surface reading only the verb would label one the other.

**One `Ops` runs one streamed operation at a time, and the engine is where that is enforced.** The `Started` handle carries the only cancel a running operation has, so a second launch replacing it leaves the first invisible and uncancellable for the rest of the session, still deleting at reference scale. `Start` and `Retry` take a launch gate and the stream frees it. A refused launch returns `ErrBusy` and leaves the confirmation unspent. A `Summary` likewise authorises one re-attempt through a single-use cell it shares by value, exactly as a `Confirmed` authorises one execution ([ADR-0019](./0019-ops-plan-and-confirmed.md)). Both belong here rather than in a surface, because [run-lifecycle](../features/run-lifecycle/requirements.md) and [storage-reclamation](../features/storage-reclamation/requirements.md) each add a launcher and an invariant enforced by one pane's state is enforced nowhere.

**A deletion holds R29's log exclusively, whichever entry it came in through.** Two deletion walks at once open two append handles to one file, each tracking size from its own `Stat`, so one handle's rotation renames the file the other is appending to and a generation can vanish. That file is the one thing recoverable from nowhere else, so a second concurrent deletion refuses to start and names the log, exactly as an unwritable log makes it refuse. Non-deletion walks write no log and are not gated: a Workflow toggle beside a running Purge touches nothing shared.

**The gate is in-process, and two `gh-runs` processes purging at once would still split the log.** It is an atomic in one `Ops`, so it holds across the surfaces one binary runs and no further. Closing that would mean a lock file under the state directory, which is a second thing to reason about when it is stale and a file nothing reads that other code would then have to read, against [ADR-0006](./0006-stateless-bulk-jobs.md)'s amendment. Two concurrent Purges are also a thing a person does deliberately rather than a race a surface can cause, and the rate governor is per process too, so a second process already spends a Budget the first cannot see. Recorded here because an unrecorded scope is how a guard gets mistaken for a stronger one than it is.

**The second deletion caller does not exist yet, and the gate is here anyway.** As built, the Feed's Purge is the only in-process launcher: `logview`'s and `storage`'s planners stop at `Plan`, and neither reaches `Execute`. So this gate guards nothing today. It is here because the callers that make it reachable are named and queued ([log-viewer](../features/log-viewer/requirements.md) R17's log deletion, [storage-reclamation](../features/storage-reclamation/requirements.md) R17's), and because the harm it prevents is silent and permanent: a split log is discovered by a person asking what a Purge destroyed, long after the answer stopped being recoverable. An unreachable guard costs one CAS per deletion. Stating that it is unreachable is the point, because a guard whose justification claims a caller that does not exist is how a doc and a tree start disagreeing.

**The root discards a frame from a stream it has replaced.** The gate is freed just before the terminal frame goes out, so that a surface acting on that frame is not refused by a gate the walk has finished with. That leaves a narrow window in which a new operation is launched while the finished one's last frame is still in flight, and applying it would mark the new operation finished before it had deleted anything. The root tags each frame with the channel it came from and drops the ones that do not match, which is the detail pane's discard rule one level up. The tag is the root's alone: the tabs receive the bare `ops.Progress` this catalog names.
