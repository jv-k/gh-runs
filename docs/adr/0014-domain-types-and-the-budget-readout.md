...
**`WorkflowState` was added when [run-detail](../features/run-detail/requirements.md) R8's marker was wired (#57), on the same argument and off the same join.** The run object carries no Workflow state key either, so a Run whose Workflow was deleted is indistinguishable from one whose Workflow still exists until `WorkflowID` is joined against the repository's Workflow list. That is the join this section already describes, resolving a second field from the same lookup, in the same place, at no second cost. The empty `State` is an unresolved join and reads as not-deleted, so a Run is never marked Orphaned on the strength of a lookup that was never made. Stamping it is what lets [workflow-management](../features/workflow-management/requirements.md) R11 label an Orphaned Run wherever that Run appears, rather than each surface repeating the join and eventually disagreeing.
