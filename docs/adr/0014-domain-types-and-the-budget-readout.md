...
**`EffectiveStart` lives on the type so the fallback exists exactly once.** The Feed sorts by it and the filter engine's date predicates read it, while cli's `startedAt` column emits the raw field rather than the fallback, exactly as gh does. Without one home, two of those three consumers reimplement R8's fallback and eventually disagree.
