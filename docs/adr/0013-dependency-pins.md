...
| `golang.org/x/sys` | `v0.46.0` | Ordinary, promoted to direct for the lock, MVS-forced to `v0.46.0` by the direct bubbletea and lipgloss requires. See below |
...
**Pinning bubbles v2.1.1 raises two indirect lines through MVS.** Its own `go.mod` requires `golang.org/x/sys v0.45.0` and `github.com/lucasb-eyer/go-colorful v1.4.0`. Both sat lower before bubbles arrived, `x/sys` at `v0.31.0` (from go-gh and `x/term`) and go-colorful at `v1.2.0` (from go-gh and termenv), and MVS takes the maximum, so both rose the moment bubbles landed. Adding the direct `charm.land/bubbletea/v2` and `charm.land/lipgloss/v2` requires (the TUI stage below) raised `x/sys` once more, from bubbles' `v0.45.0` to `v0.46.0`: bubbletea v2.0.8, lipgloss v2.0.5 and their `github.com/charmbracelet/ultraviolet` dependency each require `v0.46.0`, and MVS again takes the maximum. None is a `go get -u` drift, all are tidy-clean and verified, and all stay Ordinary tier. `x/sys` is the one the pin table records, now at `v0.46.0`, because it is also promoted to direct for the Windows lock below. go-colorful stays indirect at `v1.4.0` and needs no line of its own.
