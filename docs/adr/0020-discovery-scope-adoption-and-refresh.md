...
**The 403 is read through the governor, never from the status code.** A secondary-limit 403 arrives with a healthy `x-ratelimit-remaining`, so the status cannot tell rate limiting from authorization. [rate-governor](../features/rate-governor/requirements.md) open question 1 measured the discriminator, the classifier is the one issue [#68](https://github.com/jv-k/gh-runs/issues/68) closed, and R14 there exposes the verdict per response so a consumer never re-derives it. Discovery reads that verdict, so a burst meeting a secondary limit can never retire the account wholesale. An unclassified response is not definitive, so it retires nothing.

**That last sentence used to read "an unclassified response reports false, which is the safe direction here", and reporting false is the opposite of what it needed.** `RateLimitedHeaders` returns false both for a response the governor classified as authorization and for headers it never saw, so a consumer computing "definitive" as "not rate limiting" gets **true** for an unclassified 403 and retires a repository on a verdict nobody gave. The conclusion was right and the reason did not reach it. Discovery therefore asks two questions rather than one: `governor.ClassifiedHeaders` reports whether a verdict exists, and `RateLimitedHeaders` reports what it was, and a 403 is definitive only when the first is true and the second is false. Found in review of the code this section asked for.
