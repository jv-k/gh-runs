...
The rule is therefore about the response's shape rather than about the feature asking for it. **A request whose response is an API resource enters at the store. A request that follows a redirect to a signed URL enters at the governor.** The Artifact download ([storage-reclamation](../features/storage-reclamation/requirements.md) R13) is the first caller of the second kind. The whole-Run log archive export ([log-viewer](../features/log-viewer/requirements.md) R11) is the second, and it entered at the store until [issue 89](https://github.com/jv-k/gh-runs/issues/89) moved it, which was a defect rather than a decision anyone took. The per-Job log fetch ([log-viewer](../features/log-viewer/requirements.md) R1) is the third, and it entered at the store until [issue 153](https://github.com/jv-k/gh-runs/issues/153) moved it, for the same reason and by the same oversight. Those three are the whole population today. Every other request in the tool answers with an API resource and enters at the store.

**Amended 2026-07-27 (#153): the test is the signed URL, not the size.** This rule first read "a request whose response is a **binary download**", and the per-Job log fetch survived both earlier corrections because it fails that test and passes this one. Its body is plain text and small enough to look like an ordinary read, and the comment above its wiring said exactly that. What matters is that it arrives from a redirect to a signed URL, so the entry the store writes is addressed by something that expires in about a minute and no later request can carry. That is true of four kilobytes of log text and of a gigabyte of zip alike. Size decides how expensive the mistake is. The redirect decides whether it is one.

**The store now also refuses on its own, and that does not retire this rule.** [local-store](../features/local-store/requirements.md) R25 gives `persist` an 8 MiB per-entry ceiling above which it declines to buffer or to save ([ADR-0017](./0017-the-local-store-on-disk-contract.md)). The two are defence in depth rather than alternatives. This rule is an optimisation once the ceiling exists, because a download that never enters the store costs nothing at all, where one that enters and is declined still costs a bounded read. The ceiling is the backstop, because choosing the entry point correctly is a decision at every call site and this convention has been got wrong three times.
