...
v2 is a ground-up Go rewrite. **The 2.0.0 stages are built in Go**: the stage 0 floor (`main.go`, `domain`, `clock`, `config`, `ghclient`) and the `store` and `governor` transport chain, up through the tabs, the panes and the Settings view ([docs/BUILD-ORDER.md](docs/BUILD-ORDER.md) stage 13). The tree compiles. notifications (stage 14) stays deferred to 2.1, and what remains for 2.0.0 is wiring work. The config-to-consumer gaps #73 tracked are closed, R7's pin among them: `config.Pin` reaches `scheduler.SetPinned` from `main.go` and prices against the ceiling. Two gaps are open and both were re-checked against the tree rather than against this paragraph. `discovery.Discover` still has no caller outside its own package, so repo-discovery R22's session adoption and R14's `GH_TOKEN` instruction are written, tested and unreached (#100). Nothing ever deletes a discovery record or clears `Known`, so the poll set never shrinks and neither of the Feed's prunes can fire (#115). v1 was a bash script and now lives only at the `v1.0.7` tag. If you are looking for `delete-workflow-runs.sh`, it is not missing, it left main deliberately.
